...
class TestThumbprintValidator:
    ...
    def test_compare_thumbprints_critical_changes(self):
        ...
        changed = PageThumbprint(
            ...
            form_elements=[
                FormElement(name="new_field", id="new_id", type="text")
            ],  # Critical change
        ...
        assert len(differences["critical_changes"]) > 0
        assert any(
            "ViewState presence changed" in change
            for change in differences["critical_changes"]
        )
        assert any(
            "ViewStateGenerator changed" in change
            for change in differences["critical_changes"]
        )
        assert any(
            "form elements" in change for change in differences["critical_changes"]
        )
...
class TestValidatedFFIECDownloader:
    ...
    def test_validated_downloader_initialization(
        self, mock_validator_class, mock_downloader_class
    ):
    ...
    @patch("ffiec_data_collector.downloader.FFIECDownloader")
    @patch("ffiec_data_collector.thumbprint.ThumbprintValidator")
    def test_validated_download_success(
        self, mock_validator_class, mock_downloader_class
    ):
        ...
        mock_validator.validate.return_value = {"valid": True, "warnings": []}
    ...
    @patch("ffiec_data_collector.downloader.FFIECDownloader")
    @patch("ffiec_data_collector.thumbprint.ThumbprintValidator")
    def test_validated_download_validation_failure(
        self, mock_validator_class, mock_downloader_class
    ):
    ...
    @patch("ffiec_data_collector.downloader.FFIECDownloader")
    @patch("ffiec_data_collector.thumbprint.ThumbprintValidator")
    def test_validated_download_skip_validation(
        self, mock_validator_class, mock_downloader_class
    ):
