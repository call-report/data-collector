...
from ffiec_data_collector import (
    FFIECDownloader, ValidatedFFIECDownloader, Product, FileFormat,
    ThumbprintValidator, WebpageChangeException
)
...
class TestThumbprintValidatorIntegration:
    ...
    def test_capture_real_thumbprint(self):
        ...
        assert thumbprint.url == "https://cdr.ffiec.gov/public/pws/downloadbulkdata.aspx"
    ...
    def test_validated_download_with_real_validation(self):
        ...
        try:
            content = self.validated_downloader.download(
                product=Product.CALL_SINGLE,
                period=self.validated_downloader.downloader.select_product(Product.CALL_SINGLE)[0],
                format=FileFormat.XBRL,
                save_to_disk=False
            )
            ...
            pytest.skip("Website structure has changed - validation system working correctly")
