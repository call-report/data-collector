...
class ThumbprintValidator:
    ...
    def capture_thumbprint(self, url: str, page_type: str = "bulk_download") -> PageThumbprint:
        """
        ...
        thumbprint = PageThumbprint(
            ...
            viewstate_generator_present=bool(re.search(r'name="__VIEWSTATEGENERATOR"', html))
        )
    ...
    def _extract_form_elements(self, html: str) -> List[FormElement]:
        ...
        selects = re.findall(r'<select[^>]*name="([^"]+)"[^>]*id="([^"]+)"[^>]*>(.*?)</select>', 
                             html, re.DOTALL)
        for name, id_val, content in selects:
            options = re.findall(r'value="([^"]+)"', content)
            elements.append(FormElement(name=name, id=id_val, type="select", options=options[:5]))
        
        # Extract input elements
        inputs = re.findall(r'<input[^>]*type="([^"]+)"[^>]*name="([^"]+)"[^>]*id="([^"]+)"', html)
    ...
    def _extract_products(self, html: str) -> List[Dict[str, str]]:
        ...
        if listbox_match:
            options = re.findall(r'value="([^"]+)"[^>]*>([^<]+)</option>', listbox_match.group(1))
    ...
    def _compare_thumbprints(self, stored: PageThumbprint, current: PageThumbprint) -> Dict[str, List[str]]:
        """
        ...
        if stored.viewstate_present != current.viewstate_present:
            critical_changes.append(f"ViewState presence changed: {stored.viewstate_present} -> {current.viewstate_present}")
        
        if stored.viewstate_generator_value != current.viewstate_generator_value:
            critical_changes.append(f"ViewStateGenerator changed: {stored.viewstate_generator_value} -> {current.viewstate_generator_value}")
        
        # Critical: Form element changes
        stored_elements = {(e.name, e.id, e.type) for e in (stored.form_elements or [])}
        current_elements = {(e.name, e.id, e.type) for e in (current.form_elements or [])}
        ...
        if stored.uses_dopostback != current.uses_dopostback:
            warnings.append(f"__doPostBack usage changed: {stored.uses_dopostback} -> {current.uses_dopostback}")
...
class ValidatedFFIECDownloader:
    ...
    def __init__(self, download_dir: Optional[Path] = None, skip_validation: bool = False):
        """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("  python ffiec_thumbprint.py validate - Validate against stored thumbprints")
